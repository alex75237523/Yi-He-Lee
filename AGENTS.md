...
>
> **2026-07-13 正式更正比較方向**：2026-07-12 曾記載「進場價/平均價」與「現價」必須同時大於或等於均價才成立；此方向已被本次正式更正取代。現行規則為 MA5／MA20／MA120 任一均價只要 `均價 >= 進場價/平均價` 或 `均價 >= 現價` 其中一項成立即通知；MA60 仍只保存與顯示、不參與觸發。「進場價/平均價」與「現價」仍是兩個獨立欄位，任一欄位無效時不得判斷並須保留對應異常通知。詳見 `docs/01_需求與規則.md`、`docs/05_異動紀錄.md`。
