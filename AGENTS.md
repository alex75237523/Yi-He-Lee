...
>
> **2026-07-19 正式策略（現行、取代所有舊均價規則）**：唯一通知條件為 `進場價/平均價 > MA20 AND 現價 < MA5`（複合條件，必須兩項同時成立；嚴格大於／小於，相等不觸發）。2026-07-11／07-12／07-13 的「MA5／MA20／MA120 任一均價與進場價/平均價或現價比較」規則已作廢；MA60／MA120 只保存與顯示、不參與觸發。`進場價/平均價`、`現價`、`MA5`、`MA20` 任一無效或缺少即無法判斷並保留對應異常。盤中通知去重改為單一 `MaWindow=0` 複合狀態。分支 `fix/entry-above-ma20-current-below-ma5-alert`；`dotnet test` 通過 329／329（另 4 個 Integration 測試）。詳見 `docs/01_需求與規則.md`、`docs/05_異動紀錄.md`。
