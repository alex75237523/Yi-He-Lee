...
- **2026-07-19 正式策略**：唯一通知條件為 `進場價/平均價 > MA20 AND 現價 < MA5`（複合條件，必須兩項同時成立；嚴格大於／小於，相等不觸發）。MA60／MA120 只保存與顯示、不參與觸發。舊的「MA5／MA20／MA120 任一均價與進場價/平均價或現價比較（OR）」規則已作廢。「現價」串接外部 DDE，「進場價/平均價」非 DDE 欄位；`進場價/平均價`、`現價`、`MA5`、`MA20` 任一無效或缺少時列入對應的異常／無法判斷清單告知，不得判斷、不得混用、不得互相代替。
...
>
> **2026-07-19 正式策略（現行）**：唯一通知條件為 `進場價/平均價 > MA20 AND 現價 < MA5`（複合條件、兩項同時成立、嚴格大於／小於）。2026-07-11／07-12／07-13 的所有「MA5／MA20／MA120 任一均價與進場價/平均價或現價比較」規則已作廢；MA60／MA120 只保存與顯示、不參與觸發。盤中通知去重改為單一 `MaWindow=0` 複合狀態。詳見 `docs/01_需求與規則.md`、`docs/05_異動紀錄.md`。
