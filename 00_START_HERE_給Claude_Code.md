...
- 因此不可只靠「自持股」標題。
- 應以表頭結構辨識，必須同時找到：代號／代碼、股名／名稱、進場價/平均價、現價；張數可作輔助欄位。「進場價/平均價」與「現價」是兩個完全獨立的欄位，「現價」串接外部 DDE、「進場價/平均價」則不是（2026-07-12 恢復並取代 2026-07-11「原『進場價／平均價』為口誤」的錯誤更正）。
...
### 9. 均價策略（2026-07-19 正式更新）

每一筆有效持股，以股票代碼比對當日技術資料。

**唯一的通知條件是下列複合條件，且必須兩項同時成立才通知：**
...
程式語意必須明確等同 `entryAveragePrice > movingAverage20 && currentPrice < movingAverage5`。

> **舊規則（已作廢）**：2026-07-11「單一現價比較」、2026-07-12「雙價格 AND」、2026-07-13「MA5／MA20／MA120
> 任一均價 >= 進場價/平均價或現價（OR）」規則，皆已由 2026-07-19 新複合條件取代，不得再套用。
>
> 「現價」串接外部 DDE（看盤軟體），「進場價/平均價」不是 DDE 欄位。任一欄位儲存格為 #N/A 等錯誤值、
> 空白、0、負數或無法解析的文字時，該持股不得判斷、不得混用、不得以其他價格（含對方欄位、官方收盤價）
> 代替，必須分別列入「進場價／平均價異常」或「現價異常」清單明確告知使用者；兩者同時無效時兩個原因都
> 必須讓使用者看見。

- **邊界一律嚴格大於／小於**：`進場價/平均價 == MA20` 不觸發、`現價 == MA5` 不觸發；禁止改成 `>=`／`<=`。
- 只有其中一項成立不得觸發；整體結果只能是兩項同時成立。
- 判斷前 `進場價/平均價`、`現價`、`MA5`、`MA20` 四者都必須有效；`MA5` 或 `MA20` 缺少即無法判斷、明確顯示缺少哪一條，不得用 MA60／MA120 或其他均線替代。
- 60、120 日均價要保存及顯示，但不參與通知條件。
...
1. 符合通知條件（進場價/平均價 > MA20 且 現價 < MA5）。
2. 無法判斷。
