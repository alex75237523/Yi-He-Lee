...
每一條均價（MA5／MA20／MA120）只要小於或等於「進場價/平均價」或「現價」其中一項，任一成立就產生通知；不得再用「均價大於或等於價格」判斷通知：

```text
（5 日均價 <= 進場價/平均價 或 5 日均價 <= 現價）
或
（20 日均價 <= 進場價/平均價 或 20 日均價 <= 現價）
或
（120 日均價 <= 進場價/平均價 或 120 日均價 <= 現價）
```

> 2026-07-11 舊版（已由 2026-07-12 需求恢復取代）：曾一度只以 Excel「現價」欄位單一比較。
>
> 2026-07-12 正式恢復：「現價」串接外部 DDE（看盤軟體），「進場價/平均價」不是 DDE 欄位。
> 任一欄位儲存格為 #N/A 等錯誤值、空白、0、負數或無法解析的文字時，該持股不得判斷、不得混用、
> 不得以其他價格（含對方欄位、官方收盤價）代替，必須分別列入「進場價／平均價異常」或「現價異常」
> 清單明確告知使用者；兩者同時無效時兩個原因都必須讓使用者看見。
