...
public sealed class DailyJobServiceTests : IDisposable
{
    ...
    {
        // 進場價/平均價與現價都低於所有均線，三項條件皆不成立。
        var holding = CreateHolding("2330", "台積電", currentPrice: 1m);
        var ma = FullHistoryMovingAverage("2330", close: 900m, ma5: 890m, ma20: 880m, ma60: 870m, ma120: 860m);
    ...
    public async Task Excel均價前置列數等於DB均價快照_不含客戶診斷列()
    {
        ...
        var notTriggeredHolding = CreateHolding("5351", "鈺創", currentPrice: 1m); // 不會觸發
    ...
    // entryAveragePrice 預設為 1：本測試檔案聚焦於「現價（DDE）異常隔離」，與進場價/平均價無關；
    // 最新規則為均價 <= 進場價/平均價 OR 均價 <= 現價，因此避免預設進場價讓均價誤觸發，
    // 需要觸發的案例會讓現價本身足以觸發即可成立。
    ...
    private static CustomerHolding CreateHolding(
        ...
        decimal? entryAveragePrice = 1m, string? entryAveragePriceIssue = null) => new(
        TradeDate,
        @"C:\Data\客戶.xlsx",
        "客戶頁籤",
        "測試客戶",
