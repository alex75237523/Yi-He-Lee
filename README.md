...
- 任一條件成立即通知（2026-07-11 起比較基準改為 Excel「現價」欄位，該欄位串接外部 DDE）：
  ...
  - `20 日均價 <= 現價`
  - `120 日均價 <= 現價`
- 「現價」為外部 DDE 連結值：若儲存格為 #N/A 等錯誤值、空白、0 或無法解析的文字，該持股不得判斷、不得以其他價格代替，一律列入「現價異常」清單於畫面中央告知使用者。**DDE 現價異常只影響最後的現價比較，不得影響持股辨識、官方收盤價擷取、SQLite 寫入、歷史回補、均線計算、鉅亨交叉驗證及 Excel 均線輸出**（2026-07-11 強化並補齊自動測試，見 `docs/01_需求與規則.md`）。
- 結果寫入 `每日五日均價策略`（25 欄，含客戶、DDE狀態、DDE錯誤原因、MA5～MA120比較、整體結果、觸發條件、鉅亨驗證狀態、計算時間等診斷欄位），並於螢幕正中央顯示客戶、股票及觸發均價清單。**每一筆有效持股都會輸出一列，不論是否觸發、DDE 是否有效**（2026-07-11 起，`StrategyEvaluationService.EvaluateAll` 產生 `HoldingStrategyResult`，不再只依賴中央通知用的 `StrategyAlert` 子集合）。
