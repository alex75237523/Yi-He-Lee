...
> **2026-07-19 更新（現行正式規則）**：唯一通知條件為複合條件 `進場價/平均價 > MA20 AND 現價 < MA5`
> （必須兩項同時成立；嚴格大於／小於，相等不觸發），分支 `fix/entry-above-ma20-current-below-ma5-alert`。
> MA60／MA120 只保存與顯示、不參與觸發；`MA5`／`MA20` 缺少即無法判斷。舊的「MA5／MA20／MA120 任一均價
> `>= 進場價/平均價` 或 `>= 現價`（OR）」規則已作廢；舊 Draft PR #1 屬過期規則。詳見 `docs/05_異動紀錄.md`、`docs/06_建置驗證結果.md`。
...
- 顏色與文字排除。
- 代碼前導零。
- 複合條件（進場價/平均價 > MA20 且 現價 < MA5）兩項同時成立才通知。
- MA60／MA120 只顯示、不參與觸發。
- 無資料清單。
