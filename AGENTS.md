...
>
> **2026-07-12 需求恢復並取代上一則更正**：2026-07-11 曾一度記載「原『進場價／平均價』為口誤，正確欄位為『現價』」並將比較基準改為單一「現價」欄位——**此更正是錯誤的**，客戶實際工作簿確實同時存在「進場價/平均價」與「現價」兩個獨立欄位。使用者已正式恢復雙價格判斷：每一條均價都必須「進場價/平均價」與「現價」同時大於或等於該均價才算成立，MA60 仍只顯示不參與觸發。分支 `fix/dual-price-moving-average-condition`。詳見 `docs/01_需求與規則.md`、`docs/05_異動紀錄.md`。
