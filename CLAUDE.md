...
- 每一條均價（MA5／MA20／MA120）都必須客戶 Excel「進場價/平均價」與「現價」兩個獨立欄位同時大於或等於該均價才算成立（2026-07-12 恢復並取代 2026-07-11「現價單一欄位」更正）；「現價」串接外部 DDE，「進場價/平均價」非 DDE 欄位；任一欄位無法判讀時列入對應的異常清單告知，不得判斷、不得混用、不得互相代替。
...
>
> **2026-07-12 需求恢復並取代上一則更正**：2026-07-11 曾一度將比較基準改為單一「現價」欄位並記載「進場價／平均價為口誤」——此更正是錯誤的，客戶實際工作簿確實同時存在「進場價/平均價」與「現價」兩個獨立欄位。已正式恢復雙價格判斷（分支 `fix/dual-price-moving-average-condition`），詳見 `docs/01_需求與規則.md`、`docs/05_異動紀錄.md`。
