...
/// <summary>
/// 2026-07-11 需求恢復：客戶 Excel「進場價/平均價」與「現價」是兩個完全獨立、不得混用的欄位；
/// 每一條均價（MA5／MA20／MA120）都必須「進場價/平均價」與「現價」同時大於或等於該均價才算成立。
...
public sealed class StrategyEvaluationServiceTests
{
    ...
    [Fact]
    public void 進場價與現價都大於MA5時_MA5觸發()
    {
        var holding = CreateHolding(entryAveragePrice: 501m, currentPrice: 520m);
        var ma = CreateMovingAverage(close: 490m, ma5: 490m, ma20: 600m, ma60: 600m, ma120: 600m);
    ...
    [Fact]
    public void 進場價通過但現價未通過時_不觸發()
    {
        // 進場價 501 >= MA20(480)，但現價 460 < MA20(480)：不得觸發，禁止只取進場價判斷。
        var holding = CreateHolding(entryAveragePrice: 501m, currentPrice: 460m);
        var ma = CreateMovingAverage(close: 480m, ma5: 600m, ma20: 480m, ma60: 480m, ma120: 600m);
    ...
    [Fact]
    public void 現價通過但進場價未通過時_不觸發()
    {
        // 現價 520 >= MA20(480)，但進場價 470 < MA20(480)：不得觸發，禁止只取現價判斷。
        var holding = CreateHolding(entryAveragePrice: 470m, currentPrice: 520m);
        var ma = CreateMovingAverage(close: 480m, ma5: 600m, ma20: 480m, ma60: 480m, ma120: 600m);
    ...
    {
        var holding = CreateHolding(entryAveragePrice: 1000m, currentPrice: 1000m);
    ...
    {
        // 進場價與現價皆遠高於 MA60，但 MA5／MA20／MA120 皆未達標：不得因 MA60 而觸發。
        var holding = CreateHolding(entryAveragePrice: 1000m, currentPrice: 1000m);
        var ma = CreateMovingAverage(close: 900m, ma5: 2000m, ma20: 2000m, ma60: 1m, ma120: 2000m);
    ...
    public void 不得以收盤價代替進場價或現價()
    {
        ...
        var holding = CreateHolding(entryAveragePrice: 400m, currentPrice: 400m);
        var ma = CreateMovingAverage(close: 480m, ma5: 600m, ma20: 480m, ma60: 480m, ma120: 600m);
    ...
    public void EvaluateAll與Evaluate的均線判斷完全一致()
    {
        var holdings = new[]
        {
            CreateHolding(entryAveragePrice: 501m, currentPrice: 520m, code: "5285", name: "宜鼎"),
            CreateHolding(entryAveragePrice: 400m, currentPrice: 520m, code: "2330", name: "台積電"),
            CreateHolding(entryAveragePrice: null, currentPrice: 520m, code: "3691", name: "碩禾", entryAveragePriceIssue: "儲存格為空白，無法讀取進場價/平均價"),
        };
        var mas = new[]
        {
            CreateMovingAverage(close: 480m, ma5: 490m, ma20: 480m, ma60: 480m, ma120: 600m) with { StockCode = "5285" },
            CreateMovingAverage(close: 480m, ma5: 490m, ma20: 480m, ma60: 480m, ma120: 600m) with { StockCode = "2330" },
            CreateMovingAverage(close: 480m, ma5: 490m, ma20: 480m, ma60: 480m, ma120: 600m) with { StockCode = "3691" },
        };
    ...
    {
        var holding = CreateHolding(entryAveragePrice: 501m, currentPrice: 520m);
        var ma = CreateMovingAverage(close: 480m, ma5: 490m, ma20: 480m, ma60: 480m, ma120: 600m);
    ...
    {
        // 只有 MA5（5個有效交易日），MA20/60/120 因逐檔資料不足為 null；進場價與現價使 MA5 觸發。
        ...
        var result = new StrategyEvaluationService().Evaluate(
            TradeDate, [CreateHolding(entryAveragePrice: 100m, currentPrice: 100m)], [ma], MarketTypesFor(MarketType.Listed), TaipeiNow);
